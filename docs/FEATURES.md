...
- **Mitigating Brand Bias:** In Stage 2, all AI-generated answers are completely stripped of identifying strings (e.g., model or provider names) using regex redaction rules.
- **Algorithmic Ranking:** Models act as peer reviewers and rank the anonymized bundle. The backend uses the Borda Count method to aggregate rankings and calculate normalized scores.
...
- **Encrypted Storage:** Users input their API keys directly through the UI. These keys are Fernet-encrypted (via a `KeyVault` helper) and stored in PostgreSQL securely. No keys are hard-coded or logged.
...
- **Centralized Context:** The Decision Orchestrator invokes the Research Sub-Agent during Stage 1 and appends the resulting citation-tagged evidence digest into the prompt context identically for all Council Members.
...
- **LangSmith & Langfuse Integrations:** The platform captures deep metrics. Every LLM generation is traced, recording token ingestion, completion duration, and monetary cost.
...
## 8. Notion Archival Integration
- **Notion Publish:** Synod publishes compiled final report documents to the user's Notion workspace. It spawns the official Notion MCP server (`@notionhq/notion-mcp-server`) in a stdio subprocess to execute creation and block-append requests using OAuth tokens.
